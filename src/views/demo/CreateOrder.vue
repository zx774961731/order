<template>
  ...
  <section v-show="state.current === 1" class="choose-product">
    ...
    <n-button type="info" @click="onNext" class="f_r">
      下一步
    </n-button>
  </section>
  <section v-show="state.current === 2" class="form-container">
    <section class="user-product">
      ...
      <n-button @click="onPrevious" quaternary type="info" class="m_l_20">
        重新选择
      </n-button>
    </section>
    <NDivider />
    ...
    >
      ...
      <n-form-item label="附件：" path="fileAddress">
        附件地址
      </n-form-item>
...
watch(() => state.model.contactType, (_new, _old) => {
  if (_new === '1') {
